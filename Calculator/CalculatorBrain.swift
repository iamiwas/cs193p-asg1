...
//  Calculator
//
//  Created by Dan Christal on 2016-03-03.
        ...
        var description: String{
            get{
                ...
                case .Operand(let operand):
                    let removeZeroes = String(format: "%g", operand)
                    return removeZeroes
    ...
    private func evaluate(ops: [Op]) -> (result: Double?, remainingOps: [Op]){
        ...
        if !ops.isEmpty {
            ...
            case .UnaryOperation(_, let operation):
                let operandOperation = evaluate(remainingOps)
                if let operand = operandOperation.result {
                    return (operation(operand), operandOperation.remainingOps)
                }
